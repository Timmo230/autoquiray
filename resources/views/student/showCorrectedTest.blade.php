                ...
                <div class="question-step" id="{{ $index }}" style="display: {{ $index == 0 ? 'block' : 'none' }}">
                    <div class="card shadow border-0 rounded-4">
                        ...
                        <div class="card-body p-4">
                            ...
                            <div class="d-flex flex-column gap-3">
                                @foreach($group as $option)
                                    @php
                                        $alumnoSeleccionoEsta = collect($optionsSelected)->firstWhere('seleccionada', $option->id);
                                        $esCorrecta = ($option->id == $first->correct_option_id);
                                        $claseCss = $esCorrecta ? 'btn-correcto' : 'btn-incorrecto';
                                        $marcado = $alumnoSeleccionoEsta ? 'checked' : '';
                                    @endphp

                                    <input type="radio" 
                                        name="{{ $qId }}" 
                                        id="{{ $option->id }}" 
                                        class="btn-check {{ $claseCss }}"
                                        {{ $marcado }}
                                    ...
                                    <label for="{{ $option->id }}" class="btn btn-outline-green text-start p-3 rounded-3 d-flex align-items-center shadow-none border-light-subtle custom-option">
                                        ...
                                        @elseif($alumnoSeleccionoEsta && !$esCorrecta)
                                            <i class="bi bi-x-circle-fill ms-auto text-white"></i>
                                        @endif
                                    </label>
                                @endforeach
                            </div>
                        ...
                        <div class="card-footer bg-transparent border-0 p-4 pt-0 d-flex justify-content-between align-items-center">
                            ...
                            @else
                                <a href="{{ route('student.testType') }}" class="btn btn-primary px-4 py-2 fw-bold rounded-3">
                                    Volver <i class="bi bi-check2-all ms-1"></i>
                                </a>
                            @endif
                        </div>
                    </div>
